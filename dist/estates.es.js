import { always, arityN, assign, curry, freeze, id, identicalU, isArray, isObject, pipe2U, sndU } from 'infestines';
...
var undetermined = /*#__PURE__*/freeze([]);
...
var reduceUntil = /*#__PURE__*/curry(function (done, fn, bop, sum) {
  return function (sources) {
    return reduceUntilU(done, fn, bop, sum, sources);
  };
});

var reduce = /*#__PURE__*/reduceUntil( /*#__PURE__*/always(false));

var everyU = function everyU(predicate, sources) {
  return reduceUntilU(not, predicate, sndU, true, sources);
};
...
var forEachU = function forEachU(effect, sources) {
  return reduceUntilU(ignore, effect, sndU, undefined, sources);
};
...
function map(fn, sources) {
  if (isProperty(sources)) {
    ...
    for (var k in sources) {
      ...
      if (!identicalU(_result[k], _v)) {
        if (_result === sources) _result = assign({}, _result);
        _result[k] = _v;
      }
    }
    return _result;
...
function collectAssignments(sources, value, assignments) {
  ...
  } else if (isArray(sources) && isArray(value)) {
    for (var i = 0, n = sources.length; i < n; ++i) {
      collectAssignments(sources[i], value[i], assignments);
    }
  } else if (isObject(sources) && isObject(value)) {
    for (var k in sources) {
      collectAssignments(sources[k], value[k], assignments);
    }
  } else if (!identicalU(isProperty(sources) ? valueOf(sources) : sources, value)) {
...
function sinksForEach(effect, sinks) {
  if (isSink(sinks)) effect(sinks);else if (isArray(sinks)) sinks.forEach(effect);
}
...
var set$1 = /*#__PURE__*/curry(function (settable, value) {
  return withSubscriber(function (settable) {
    setU(settable, value);
  }, settable);
});

var modify = /*#__PURE__*/curry(function (settable, fn) {
  return withSubscriber(function (settable) {
...
var someProperty = /*#__PURE__*/reduceUntil(id, id, sndU, undefined);

var someMutable = /*#__PURE__*/reduceUntil(isMutable, id, sndU, undefined);
...
function propagateNext(value, property) {
  if (!identicalU(property.value, value)) {
...
var debounce = /*#__PURE__*/curry(function (ms, sources) {
...
var FromPromise = /*#__PURE__*/function (_Property) {
  ...
  function FromPromise(promise) {
    ...
    promise.then(then, pipe2U(failure, then));
    return _this;
  }
...
var map$1 = /*#__PURE__*/curry(function (fn, arg) {
  return isVarying(arg) ? new Lift1(fn, arg) : fn(arg);
});
...
var on = /*#__PURE__*/curry(function (effect, sources) {
...
var onValue = /*#__PURE__*/curry(function (effect, sources) {
  if (isVarying(sources)) {
...
var skipFirst = /*#__PURE__*/curry(function (n, sources) {
...
var skipUnless = /*#__PURE__*/curry(function (predicate, sources) {
...
var takeFirst = /*#__PURE__*/curry(function (n, sources) {
...
var tap = /*#__PURE__*/curry(function (effect, sources) {
  if (isVarying(sources)) {
...
var StaticView = /*#__PURE__*/function (_Mutable) {
  ...
  createClass(StaticView, [{
    key: 'collectAssignments',
    value: function collectAssignments$$1(value, assignments) {
      collectAssignments(this.sources, set(this.lens, value, render(this.sources)), assignments);
    }
  }, {
    key: 'next',
    value: function next(data) {
      propagateNext(get(this.lens, data), this);
    }
  }, {
    key: 'complete',
    value: function complete() {
...
var DynamicView = /*#__PURE__*/function (_Mutable2) {
  ...
  createClass(DynamicView, [{
    key: 'collectAssignments',
    value: function collectAssignments$$1(value, assignments) {
      collectAssignments(this.sources[1], set(render(this.sources[0]), value, render(this.sources[1])), assignments);
    }
  }, {
    key: 'next',
    value: function next(input) {
      propagateNext(get(input[0], input[1]), this);
    }
  }]);
  return DynamicView;
}(Mutable);

var view = /*#__PURE__*/curry(function (lens, data) {
  return isVarying(lens) ? isSettable(data) ? new DynamicView(lens, data) : new Lift(get, [lens, data]) : isSettable(data) ? new StaticView(lens, data) : isVarying(data) ? new Lift1(get(lens), data) : get(lens, data);
});

export { atom, debounce, failure, fromPromise, get$1 as get, isDetermined, isFailure, isSettable, isVarying, lift, map$1 as map, modify, on, onValue, reasonOf, set$1 as set, skipFirst, skipUnless, takeFirst, tap, unsubscribe, variable, view };
